...
public class Graph {
    ...
    private int[] findAncestors(int start, int destination) {// nummering van
        ...
        while (huidig != destination) {
            for (int j = 1; j <= this.getAantalKnopen(); j++) {
                if (rechtstreekseVerbinding(huidig, j) && ancestors[j - 1] == infty) {
                    queue.add(j);
                    ancestors[j - 1] = huidig;
                }
            }
            if (!queue.isEmpty())
                huidig = queue.remove();
    ...
    public List<Integer> findPath(int start, int destination) {
        ...
        // oefening 1.5
        int ancestor = ancestors[destination - 1];
        while (ancestor != 0 && ancestor != infty) {
            path.add(0, destination);
            destination = ancestor;
            ancestor = ancestors[destination - 1];
            if (ancestor == 0) {
                path.add(0, destination);
            }
        }
