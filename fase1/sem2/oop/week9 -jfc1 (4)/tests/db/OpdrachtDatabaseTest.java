...
public class OpdrachtDatabaseTest {
	...
	@Test
	public void sort_by_id_correct () throws Exception {
		List<Opdracht> opdrachten = new OpdrachtDatabank("Opdrachten-1.txt").getOpdrachtenGesorteerdOpId();
		Opdracht prev = opdrachten.get(0);
		boolean descending = true;
		for (int i = 1; i < opdrachten.size(); i++) {
			if (prev.getOpdrachtID() > opdrachten.get(i).getOpdrachtID()) {
				descending = false;
				break;
			}
		}
